---
title: "Getting Started with Adaptive Assessments in inrep"
...
output:
  html_document:
    ...
    css: styles.css
vignette: >
  %\VignetteIndexEntry{Getting Started with Adaptive Assessments in inrep}
...
# Welcome to inrep
This vignette introduces adaptive assessments in **inrep** using IRT-style item banks. It shows how to define an item bank, configure a study, and run an adaptive session in Shiny.
...
When developing and testing your assessments, you can enable **debug mode** to rapidly fill forms with random data:

```{r debug_mode, eval=FALSE}
# Enable debug mode with keyboard shortcuts
launch_study(config, bfi_items, debug_mode = TRUE)
```

**Debug mode features:**
- **STRG+A** (CTRL+A): Automatically fills the current page with random responses (text, numbers, radio buttons, checkboxes, etc.)
- **STRG+Q** (CTRL+Q): Auto-fills ALL pages sequentially until the results page is reached
- A red indicator appears in the bottom-right corner showing available shortcuts
- **Use only for development/testing** - not for production!

This is extremely useful when:
- Testing your study flow and page transitions
- Debugging results calculation functions
...
launch_study(config, work_personality_items, debug_mode = TRUE)  # Debug mode enabled
```

**In many studies, you will want a custom item bank:**
...
- **Control**: You control item content and parameters
- **Updating parameters**: If needed, item parameters can be updated between runs.
...
- Parameters are included with the dataset
...
# Launch the adaptive test with our custom item bank
launch_study(config, work_personality_items, debug_mode = TRUE)
```
...
launch_study(config, math_knowledge_items, debug_mode = TRUE)
```
