...
- **Parallel Processing**: High-performance computing for large-scale assessments
...
- **Production Quality**: CRAN-ready code with extensive documentation and testing
...
Before using inrep, ensure you have:
- R version 4.0.0 or higher
- Required dependencies: TAM, shiny, ggplot2, dplyr
- For parallel processing: future, future.apply packages
- For advanced features: mirt package (optional)
...
When you run `launch_study()`, you'll see:
- A modern, responsive web interface
- Real-time ability estimation updates
- Progress indicators and item information
- Automatic stopping when criteria are met
...
# Show item bank summary
cat("\nItem Bank Summary:\n")
cat("- Total items:", nrow(bfi_items), "\n")
cat("- Response format:", "Likert scale (1-5)", "\n")
cat("- IRT model:", "Graded Response Model (GRM)", "\n")
cat("- Domains:", length(unique(bfi_items$Trait)), "personality traits\n")
```
...
- `Option1-Option5`: Response options for the item (5-point Likert scale)
- `Trait`: Personality domain (Openness, Conscientiousness, etc.)
- `Domain`: Broader category classification
...
inrep offers two estimation engines:
...
Welcome to adaptive testing with inrep. Let's get you started with your first adaptive assessment.
