...
get_help <- function(topic = NULL) {
  if (is.null(topic)) {
    ...
    message("  check_setup()         - Verify configuration")
    message("  diagnose_issue()      - Troubleshoot problems")
    message("  configuration_wizard() - Create configuration")
    message("\nHelp topics:")
...
- 'Package not found': Run install_required_packages()
- 'Invalid config': Run validate_config(config)
- 'Item bank error': Check validate_item_bank(items)
- Memory issues: Reduce item bank size or enable caching
For detailed diagnostics: diagnose_issue()
"
  )
