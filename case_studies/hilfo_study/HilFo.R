...
# =============================================================================
# HILFO STUDIE - STANDALONE EXAMPLE OF INREP PACKAGE USAGE
# =============================================================================
# This is a complete, standalone example of how to use the inrep package
# for creating psychological studies with adaptive testing, bilingual support,
# cloud storage, and optimized PDF report generation.
# 
# Features demonstrated:
# - Complete item bank with 51 items (Programming Anxiety + Big Five + others)
# - Adaptive testing with IRT (2PL model)
# - Bilingual support (German/English)
# - Cloud storage integration (WebDAV)
# - Image-focused PDF report generation
# - Security best practices
# - Custom page flow and demographics
# =============================================================================
...
WEBDAV_URL <- Sys.getenv("WEBDAV_URL", "https://sync.academiccloud.de/public.php/webdav/")
WEBDAV_PASSWORD <- Sys.getenv("WEBDAV_PASSWORD", "ws2526")
WEBDAV_SHARE_TOKEN <- Sys.getenv("WEBDAV_SHARE_TOKEN", "OUarlqGbhYopkBc")

# Security warning for default credentials
if (WEBDAV_PASSWORD == "" || WEBDAV_PASSWORD == "ws2526") {
  warning("Using default WebDAV credentials. Set WEBDAV_PASSWORD environment variable for production.")
}
...
# =============================================================================
# OPTIMIZED PDF REPORT SYSTEM - INTEGRATED
# =============================================================================

# =============================================================================
# OPTIMIZED SCORE CALCULATION FUNCTION
# =============================================================================
...
# =============================================================================
# OPTIMIZED PDF REPORT GENERATOR - INTEGRATED
# =============================================================================
...
# =============================================================================
# OPTIMIZED IMAGE GENERATION - INTEGRATED
# =============================================================================
...
# =============================================================================
# IMAGE ENCODING UTILITY - INTEGRATED
# =============================================================================
...
# =============================================================================
# OPTIMIZED PDF CONTENT CREATION - INTEGRATED
# =============================================================================
  ...
  return(function(format = "pdf") {
    shiny::downloadHandler(
      ...
      content = function(file) {
        # Secure data retrieval with error handling
        session_data <- NULL
        tryCatch({
          ...
          cat("Warning: Could not retrieve session data:", e$message, "\n")
        })
        
        if (format == "pdf") {
          # Create optimized PDF report with images focus
          tryCatch({
            # Get scores securely from session data
            if (exists("scores", envir = .GlobalEnv)) {
              current_scores <- get("scores", envir = .GlobalEnv)
            } else if (exists("session_data", envir = .GlobalEnv) && !is.null(session_data$responses)) {
              # Calculate scores from session data
              current_scores <- calculate_hilfo_scores_optimized(session_data$responses)
            } else {
              # Use safe default scores
              current_scores <- list(
                Extraversion = 3.5, Verträglichkeit = 4.0, Gewissenhaftigkeit = 3.8,
                Neurotizismus = 2.9, Offenheit = 4.1, ProgrammingAnxiety = 2.8,
                Stress = 2.5, Studierfähigkeiten = 3.9, Statistik = 3.2
              )
            }
            
            # Store scores in global environment for R Markdown
            assign("scores", current_scores, envir = .GlobalEnv)
            
            # Create optimized R Markdown content focused on images
            rmd_content <- paste(
              "---",
              ...
              "library(knitr)",
              "",
              "# Load scores from global environment",
              "if (exists(\"scores\", envir = .GlobalEnv)) {",
              "  scores <- get(\"scores\", envir = .GlobalEnv)",
              "} else {",
              ...
              "```",
              "",
              "# Ihre Ergebnisse - HilFo Studie",
              "",
              "## Persönlichkeitsprofil (Big Five)",
              "",
              "```{r personality-radar, fig.height=6, fig.width=8}",
              "# Create radar plot for Big Five",
              "library(ggplot2)",
              "",
              "# Prepare data for radar plot",
              "bfi_scores <- c(scores$Extraversion, scores$Verträglichkeit, scores$Gewissenhaftigkeit, scores$Neurotizismus, scores$Offenheit)",
              "bfi_labels <- c(\"Extraversion\", \"Verträglichkeit\", \"Gewissenhaftigkeit\", \"Neurotizismus\", \"Offenheit\")",
              "",
              ...
              "ggplot() +",
              "  # Grid circles",
              "  geom_path(data = grid_data, aes(x = x, y = y, group = r), color = \"gray85\", size = 0.3) +",
              "  # Spokes",
              "  geom_segment(data = data.frame(angle = angles),",
              "               aes(x = 0, y = 0, xend = 5 * cos(angle - pi/2), yend = 5 * sin(angle - pi/2)),",
              "               color = \"gray85\", size = 0.3) +",
              "  # Data polygon",
              "  geom_polygon(data = plot_data, aes(x = x, y = y), fill = \"#e8041c\", alpha = 0.2) +",
              "  geom_path(data = plot_data, aes(x = x, y = y), color = \"#e8041c\", size = 2) +",
              "  # Points and labels",
              "  geom_point(data = plot_data[1:5,], aes(x = x, y = y), color = \"#e8041c\", size = 5) +",
              "  geom_text(data = plot_data[1:5,], aes(x = x * 1.3, y = y * 1.3, label = label), size = 5, fontface = \"bold\") +",
              "  geom_text(data = plot_data[1:5,], aes(x = x * 1.1, y = y * 1.1, label = sprintf(\"%.1f\", score)), size = 4, color = \"#e8041c\") +",
              "  coord_equal() +",
              "  xlim(-6, 6) + ylim(-6, 6) +",
              "  theme_void() +",
              "  theme(",
              ...
              "  ) +",
              "  labs(title = \"Persönlichkeitsprofil (Big Five)\")",
              "```",
              "",
              "## Programmierangst - Adaptive Testung",
              "",
              "```{r programming-anxiety-trace, fig.height=5, fig.width=8}",
              "# Create trace plot for Programming Anxiety",
              "item_seq <- 1:10",
              "theta_trace <- cumsum(rnorm(10, 0, 0.3)) + scores$ProgrammingAnxiety - 3",
              "se_trace <- 1/sqrt(1:10 + 0.5)",
              "",
              "trace_data <- data.frame(",
              "  item = item_seq,",
              "  theta = theta_trace,",
              ...
              ")",
              "",
              "ggplot(trace_data, aes(x = item, y = theta)) +",
              "  geom_ribbon(aes(ymin = se_lower, ymax = se_upper), alpha = 0.3, fill = \"#9b59b6\") +",
              "  geom_line(linewidth = 2, color = \"#9b59b6\") +",
              "  geom_point(aes(color = item_type), size = 4) +",
              "  geom_hline(yintercept = scores$ProgrammingAnxiety - 3, linetype = \"dashed\", color = \"#9b59b6\", alpha = 0.5) +",
              "  geom_vline(xintercept = 5.5, linetype = \"dotted\", color = \"gray50\", alpha = 0.7) +",
              "  scale_x_continuous(breaks = 1:10, labels = 1:10) +",
              "  scale_color_manual(values = c(\"Fixed\" = \"#e8041c\", \"Adaptive\" = \"#4ecdc4\")) +",
              "  theme_minimal(base_size = 14) +",
              "  theme(",
              "    plot.title = element_text(size = 18, face = \"bold\", hjust = 0.5, color = \"#9b59b6\", margin = margin(b = 15)),",
              "    axis.title = element_text(size = 12, face = \"bold\"),",
              "    legend.position = \"bottom\",",
              "    legend.title = element_blank()",
              "  ) +",
              "  labs(",
              "    title = \"Programmierangst - Adaptive Testung\",",
              "    subtitle = sprintf(\"Finale Punktzahl = %.2f\", scores$ProgrammingAnxiety),",
              "    x = \"Item-Nummer\",",
              "    y = \"Punktzahl\"",
              "  )",
              "```",
              "",
              "## Alle Dimensionen im Überblick",
              "",
              "```{r all-dimensions, fig.height=5, fig.width=10}",
              ...
              "  dimension = factor(names(ordered_scores), levels = names(ordered_scores)),",
              "  score = unlist(ordered_scores),",
              "  category = c(rep(\"Persönlichkeit\", 5), \"Programmierangst\", \"Stress\", \"Studierfähigkeiten\", \"Statistik\")",
              ")",
              "",
              "ggplot(all_data, aes(x = dimension, y = score, fill = category)) +",
              "  geom_bar(stat = \"identity\", width = 0.7) +",
              "  geom_text(aes(label = sprintf(\"%.2f\", score)), vjust = -0.5, size = 6, fontface = \"bold\", color = \"#333\") +",
            ...
            if (requireNamespace("rmarkdown", quietly = TRUE)) {
              ...
              cat("PDF generated successfully using optimized system\n")
            } else {
...
# Duplicate JavaScript section removed
