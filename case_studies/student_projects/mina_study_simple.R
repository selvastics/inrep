    ...
    render_function = function(input, output, session, rv) {
      shiny::div(
        ...
        shiny::div(style = "padding: 16px;",
                   ...
                   shiny::numericInput("demo_Semester", "Bitte geben Sie Ihr aktuelles Semester an.", value = NA, min = 1, max = 100),
...
# =============================================================================
# DATA SAVE FUNCTION - Upload CSV to WebDAV
# =============================================================================
save_to_cloud <- function(data = NULL, filename = NULL, ...) {
  ...
  tryCatch({
    # Extract share token if using Nextcloud/ownCloud public share format
    share_token <- NULL
    upload_url_base <- WEBDAV_URL
    
    if (grepl("index.php/s/", WEBDAV_URL)) {
      share_token <- gsub(".*index.php/s/([^/]+).*", "\\1", WEBDAV_URL)
      base_url <- gsub("(https?://[^/]+).*", "\\1", WEBDAV_URL)
      upload_url_base <- paste0(base_url, "/public.php/webdav/")
    }
    
    # Ensure URL ends with /
    if (!grepl("/$", upload_url_base)) upload_url_base <- paste0(upload_url_base, "/")
    upload_url <- paste0(upload_url_base, filename)
    
    # Save locally first (temp file)
    local_file <- tempfile(fileext = ".csv")
    write.csv(data, local_file, row.names = FALSE, na = "")
    
    # Upload to WebDAV
    if (requireNamespace("httr", quietly = TRUE)) {
      # Set up authentication
      auth <- if (!is.null(share_token) && nzchar(share_token)) {
        httr::authenticate(user = share_token, password = WEBDAV_PASSWORD, type = "basic")
      } else if (!is.null(WEBDAV_PASSWORD) && nzchar(WEBDAV_PASSWORD)) {
        httr::authenticate(user = "", password = WEBDAV_PASSWORD, type = "basic")
      } else {
        NULL
      }
      ...
      if (status_code %in% c(200, 201, 204)) {
        message("✓ Data successfully uploaded to WebDAV!")
      ...
      message("Warning: 'httr' package not available for WebDAV upload")
      unlink(local_file)
      return(FALSE)
    }
    ...
    message("Error saving data to WebDAV: ", e$message)
    return(FALSE)
  })
}
...
create_mina_report <- function(responses, item_bank, demographics = NULL, rv = NULL, input = NULL, ...) {
  ...
  # Capture demographics - prefer rv$demo_data; fallback to demographics arg; then input
  if (!is.null(rv$demo_data) && is.list(rv$demo_data) && length(rv$demo_data) > 0) {
    data$Age <- if (!is.null(rv$demo_data$Age)) rv$demo_data$Age else NA
    data$Gender <- if (!is.null(rv$demo_data$Gender)) rv$demo_data$Gender else NA
    data$StudyProgram <- if (!is.null(rv$demo_data$StudyProgram)) rv$demo_data$StudyProgram else NA
    data$Semester <- if (!is.null(rv$demo_data$Semester)) rv$demo_data$Semester else NA
    data$ScreenTime <- if (!is.null(rv$demo_data$ScreenTime)) rv$demo_data$ScreenTime else NA
    data$SocialMedia <- if (!is.null(rv$demo_data$SocialMedia)) rv$demo_data$SocialMedia else NA
  } else if (!is.null(demographics) && is.list(demographics) && length(demographics) > 0) {
    data$Age <- if (!is.null(demographics$Age)) demographics$Age else NA
    data$Gender <- if (!is.null(demographics$Gender)) demographics$Gender else NA
    data$StudyProgram <- if (!is.null(demographics$StudyProgram)) demographics$StudyProgram else NA
    data$Semester <- if (!is.null(demographics$Semester)) demographics$Semester else NA
    data$ScreenTime <- if (!is.null(demographics$ScreenTime)) demographics$ScreenTime else NA
    data$SocialMedia <- if (!is.null(demographics$SocialMedia)) demographics$SocialMedia else NA
  } else {
    # Fallback: try input directly
    if (!is.null(input)) {
      data$Age <- input$demo_Age %||% NA
      data$Gender <- input$demo_Gender %||% NA
      data$StudyProgram <- input$demo_StudyProgram %||% NA
      data$Semester <- input$demo_Semester %||% NA
      data$ScreenTime <- input$demo_ScreenTime %||% NA
      data$SocialMedia <- input$demo_SocialMedia %||% NA
    } else {
      data$Age <- NA
      data$Gender <- NA
      data$StudyProgram <- NA
      data$Semester <- NA
      data$ScreenTime <- NA
      data$SocialMedia <- NA
    }
  ...
  data$open_O1 <- if (!is.null(rv$open_O1)) rv$open_O1 else if (!is.null(input$open_O1)) input$open_O1 else NA
  
  # Add response labels for reference
  data$response_scale <- "1 = stimme gar nicht zu … 7 = stimme voll zu"
  
  # Update rv$demo_data to ensure it's properly set
  ...
  # Save to WebDAV cloud storage (using inrep pattern like UMA study)
  timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
  filename <- paste0("MINA_", timestamp, "_", session_id, ".csv")
