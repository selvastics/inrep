...
custom_page_flow <- list(
  ...
  list(
    ...
    content = paste0(
      ...
      '<script>',
      'console.log("EMBEDDED JS LOADING...");',
      'var translations = {',
      '"Willkommen zur HilFo Studie": "Welcome to the HilFo Study",',
      '"Liebe Studierende,": "Dear Students,",',
      '"In den Übungen zu den statistischen Verfahren wollen wir mit anschaulichen Daten arbeiten, die von Ihnen selbst stammen.": "In the statistical methods exercises, we want to work with illustrative data that comes from you.",',
      '"Deswegen wollen wir ein paar Dinge von Ihnen erfahren.": "Therefore, we want to learn a few things about you.",',
      '"Da wir verschiedene Auswertungen ermöglichen wollen, deckt der Fragebogen verschiedene Themenbereiche ab": "Since we want to enable various analyses, the questionnaire covers different topic areas",',
      '"die voneinander teilweise unabhängig sind.": "that are partially independent of each other.",',
      '"Ihre Angaben sind dabei selbstverständlich anonym": "Your information is of course anonymous",',
      '"es wird keine personenbezogene Auswertung der Daten stattfinden.": "no personal evaluation of the data will take place.",',
      '"Die Daten werden von den Erstsemestern Psychologie im Bachelor generiert": "The data is generated by first-semester psychology bachelor students",',
      '"und in diesem Jahrgang genutzt, möglicherweise auch in späteren Jahrgängen.": "and used in this year group, possibly also in later year groups.",',
      '"Im Folgenden werden Ihnen dazu Aussagen präsentiert.": "In the following, statements will be presented to you.",',
      '"Wir bitten Sie anzugeben, inwieweit Sie diesen zustimmen.": "We ask you to indicate the extent to which you agree with them.",',
      '"Es gibt keine falschen oder richtigen Antworten.": "There are no wrong or right answers.",',
      '"Bitte beantworten Sie die Fragen so, wie es Ihrer Meinung am ehesten entspricht.": "Please answer the questions as they best correspond to your opinion.",',
      '"Die Befragung dauert etwa 10-15 Minuten.": "The survey takes about 10-15 minutes.",',
      '"Einverständniserklärung": "Consent Declaration",',
      '"Ich bin mit der Teilnahme an der Befragung einverstanden": "I agree to participate in the survey",',
      '"Seite": "Page",',
      '"von": "of",',
      '"Geschlecht": "Gender",',
      ...
      'console.log("INITIAL LANGUAGE FROM STORAGE:", currentLang);',
      'if (currentLang === "en") {',
      'console.log("PAGE LOADED IN ENGLISH - AUTO-TRANSLATING!");',
      'setTimeout(function() {',
      'translatePage();',
      'var welcomeBtnText = document.getElementById("lang_switch_text");',
      'if (welcomeBtnText) welcomeBtnText.textContent = "Deutsche Version";',
      '}, 500);',
      '}',
      'function translatePage() {',
      'console.log("translatePage called, lang:", currentLang);',
      'if (currentLang === "en") {',
      'console.log("ENFORCING COMPLETE ENGLISH TRANSLATION OF EVERYTHING!");',
      'var translationCount = 0;',
      'for (var germanText in translations) {',
      'var englishText = translations[germanText];',
      'var allElements = document.querySelectorAll("*");',
      'for (var i = 0; i < allElements.length; i++) {',
      'var el = allElements[i];',
      'if (el.tagName !== "SCRIPT" && el.tagName !== "STYLE" && el.id !== "lang_switch" && !el.classList.contains("btn") && !el.classList.contains("action-button")) {',
      ...
      'el.textContent = el.textContent.replace(new RegExp(germanText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), englishText);',
      '} else {',
      'el.innerHTML = el.innerHTML.replace(new RegExp(germanText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), englishText);',
      '}',
      'translationCount++;',
      '}',
      'if (el.value && el.value.includes && el.value.includes(germanText)) {',
      'el.value = el.value.replace(new RegExp(germanText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), englishText);',
      'translationCount++;',
      '}',
      'if (el.placeholder && el.placeholder.includes(germanText)) {',
      'el.placeholder = el.placeholder.replace(new RegExp(germanText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), englishText);',
      'translationCount++;',
      '}',
      '}',
      '}',
      '}',
      'console.log("TRANSLATION COMPLETE: Applied", translationCount, "translations");',
      'setTimeout(function() { translatePage(); }, 1000);',
      '} else if (currentLang === "de") {',
      'console.log("REVERTING TO GERMAN - TRANSLATING BACK!");',
      'var translationCount = 0;',
      'for (var germanText in translations) {',
      'var englishText = translations[germanText];',
      'var allElements = document.querySelectorAll("*");',
      ...
      '}',
      '} else if (el.children.length === 0) {',
      'el.textContent = el.textContent.replace(new RegExp(englishText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), germanText);',
      '} else {',
      'el.innerHTML = el.innerHTML.replace(new RegExp(englishText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), germanText);',
      '}',
      'translationCount++;',
      '}',
      'if (el.value && el.value.includes && el.value.includes(englishText)) {',
      'el.value = el.value.replace(new RegExp(englishText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), germanText);',
      'translationCount++;',
      '}',
      'if (el.placeholder && el.placeholder.includes(englishText)) {',
      'el.placeholder = el.placeholder.replace(new RegExp(englishText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), germanText);',
      'translationCount++;',
      '}',
      '}',
      '}',
      '}',
      'console.log("GERMAN REVERSION COMPLETE: Applied", translationCount, "back-translations");',
      'if (translationCount === 0) {',
      'console.log("WARNING: No German translations applied! Current page might be static HTML.");',
      'console.log("Attempting direct text replacement...");',
      'var pageTitle = document.querySelector("h1, h2, .title");',
      'if (pageTitle && pageTitle.textContent.includes("Welcome")) {',
      'pageTitle.textContent = pageTitle.textContent.replace("Welcome to the HilFo Study", "Willkommen zur HilFo Studie");',
      'console.log("Direct title translation applied");',
      '}',
      '}',
      'setTimeout(function() { translatePage(); }, 1000);',
      '}',
      ...
      'return;',
      '}',
      'var langButton = document.getElementById("lang_switch");',
      ...
      'var welcomeBtnText = document.getElementById("lang_switch_text");',
      'if (welcomeBtnText) {',
      'welcomeBtnText.textContent = currentLang === "en" ? "Deutsche Version" : "English Version";',
      'console.log("Button text updated to:", welcomeBtnText.textContent);',
      '}',
      'if (typeof Shiny !== "undefined" && Shiny.setInputValue) {',
      'console.log("Setting Shiny input study_language to:", currentLang);',
      'Shiny.setInputValue("study_language", currentLang, {priority: "event"});',
      'setTimeout(function() {',
      'console.log("ENFORCING IMMEDIATE COMPREHENSIVE TRANSLATION!");',
      'translatePage();',
      'setTimeout(function() { console.log("Translation attempt 2"); translatePage(); }, 500);',
      ...
      '}, 3000);',
      '};',
      'console.log("toggleLanguage function defined:", typeof window.toggleLanguage);',
      '</script>',
