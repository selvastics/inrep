...
# Create functions to get content in the correct language
...
# Function to get demographic configs in the correct language
get_demographic_configs_for_language <- function(lang = "de") {
  configs <- demographic_configs
  
  if (lang == "en") {
    # Update all demographic configs to use English versions
    for (demo_name in names(configs)) {
      config <- configs[[demo_name]]
      
      # Update question text
      if ("question_en" %in% names(config)) {
        config$question <- config$question_en
      }
      
      # Update options
      if ("options_en" %in% names(config)) {
        config$options <- config$options_en
      }
      
      configs[[demo_name]] <- config
    }
  }
