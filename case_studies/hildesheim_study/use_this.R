...
custom_page_flow <- list(
  ...
  list(
    ...
    content = paste0(
      ...
      'var originalDataProps = {};',
      'for (var attr of el.attributes) {',
      'if (attr.name.startsWith("data-")) originalDataProps[attr.name] = attr.value;',
      '}',
      'el.innerHTML = el.innerHTML.replace(new RegExp(germanText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), englishText);',
      'if (originalOnclick) el.onclick = originalOnclick;',
      'if (originalId) el.id = originalId;',
      'if (originalClass) el.className = originalClass;',
      'for (var prop in originalDataProps) el.setAttribute(prop, originalDataProps[prop]);',
      '} else {',
      'el.innerHTML = el.innerHTML.replace(new RegExp(germanText.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&"), "g"), englishText);',
      '}',
      'translationCount++;',
      ...
      'setTimeout(function() { isToggling = false; }, 2000);',
      '};',
