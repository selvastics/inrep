...
# Using a simpler, minified approach to avoid R parsing issues
custom_js <- paste0(
  "<script>",
  "var currentLang='de';",
  "function toggleLanguage(){currentLang=currentLang==='de'?'en':'de';",
  "document.querySelectorAll('*').forEach(function(el){",
  "var t=el.textContent;",
  "if(t&&currentLang==='en'){",
  "if(t.indexOf('Weiter')>=0)el.textContent=t.replace('Weiter','Next');",
  "if(t.indexOf('Zurück')>=0)el.textContent=t.replace('Zurück','Back');",
  "if(t.indexOf('Seite')>=0)el.textContent=t.replace('Seite','Page');",
  "if(t.indexOf(' von ')>=0)el.textContent=t.replace(' von ',' of ');",
  "if(t.indexOf('Soziodemographische Angaben')>=0)el.textContent='Sociodemographic Information';",
  "if(t.indexOf('Wie alt sind Sie?')>=0)el.textContent='How old are you?';",
  "if(t.indexOf('Bachelor Psychologie')>=0)el.textContent='Bachelor Psychology';",
  "if(t.indexOf('Master Psychologie')>=0)el.textContent='Master Psychology';",
  "if(t.indexOf('weiblich oder divers')>=0)el.textContent='female or diverse';",
  "if(t.indexOf('männlich')>=0)el.textContent='male';",
  "if(t.indexOf('In welchem Studiengang befinden Sie sich?')>=0)el.textContent='Which study program are you in?';",
  "if(t.indexOf('Welches Geschlecht haben Sie?')>=0)el.textContent='What is your gender?';",
  "if(t.indexOf('Bitte beantworten Sie:')>=0)el.textContent='Please answer:';",
  "}});",
  "var btn=document.getElementById('lang_toggle');",
  "if(btn)btn.textContent=currentLang==='de'?'English':'Deutsch';",
  "if(typeof Shiny!=='undefined')Shiny.setInputValue('study_language',currentLang,{priority:'event'});",
  "}",
  "document.addEventListener('DOMContentLoaded',function(){",
  "var btn=document.createElement('button');",
  "btn.id='lang_toggle';",
  "btn.textContent='English';",
  "btn.style.cssText='position:fixed;top:10px;right:10px;z-index:9999;background:white;border:2px solid #e8041c;color:#e8041c;padding:8px 16px;border-radius:4px;cursor:pointer;';",
