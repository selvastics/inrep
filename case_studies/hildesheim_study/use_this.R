...
custom_js <- paste0(
  ...
  "  ",
  "  // COMPLETE translation dictionary with ALL German-English pairs",
  "  var dict = {",
  ...
  "  };",
  "  ",
  "  // Translation function that walks through ALL text nodes",
  ...
  "        if (currentLang === 'en' && dict[text.trim()]) {",
  "          newText = text.replace(text.trim(), dict[text.trim()]);",
  "        } else if (currentLang === 'en') {",
  "          // Try to translate parts of the text",
  "          for (var de in dict) {",
  "            if (newText.indexOf(de) !== -1) {",
  "              var regex = new RegExp(de.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&'), 'g');",
  "              newText = newText.replace(regex, dict[de]);",
  "            }",
  "          }",
  ...
  "              var regex = new RegExp(en.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&'), 'g');",
  ...
  "        if (currentLang === 'en' && dict[ph]) node.placeholder = dict[ph];",
  "        else if (currentLang === 'de') {",
  "          for (var de in dict) {",
  "            if (dict[de] === ph) { node.placeholder = de; break; }",
  "          }",
  "        }",
  "      }",
  "      if (node.title) {",
  "        var tt = node.title;",
  "        if (currentLang === 'en' && dict[tt]) node.title = dict[tt];",
  "        else if (currentLang === 'de') {",
  "          for (var de in dict) {",
  "            if (dict[de] === tt) { node.title = de; break; }",
  "          }",
  "        }",
  "      }",
  "      if (node.value && node.tagName === 'BUTTON') {",
  "        var val = node.value;",
  "        if (currentLang === 'en' && dict[val]) node.value = dict[val];",
  "        else if (currentLang === 'de') {",
  "          for (var de in dict) {",
  "            if (dict[de] === val) { node.value = de; break; }",
  "          }",
  "        }",
  "      }",
  ...
  "  if (currentLang === 'en') {",
  "    // Remove German validation prefixes when in English mode",
  "    document.querySelectorAll('.shiny-input-container').forEach(function(container) {",
  "      var labels = container.querySelectorAll('.control-label, .input-label, label');",
  "      labels.forEach(function(label) {",
  "        if (label.textContent.indexOf('Bitte beantworten Sie:') >= 0) {",
  "          label.textContent = label.textContent.replace('Bitte beantworten Sie:', 'Please answer:');",
  "        }",
  "        if (label.textContent.indexOf('Please complete the following:') >= 0) {",
  "          label.textContent = label.textContent.replace('Please complete the following:\\n', '');",
  "        }",
  "      });",
  "    });",
  "  }",
  ...
  "  ",
  "  // Handle select options specifically",
  "  document.querySelectorAll('select').forEach(function(select) {",
  "    for (var i = 0; i < select.options.length; i++) {",
  "      var opt = select.options[i];",
  "      if (currentLang === 'en' && dict[opt.text]) {",
  "        opt.text = dict[opt.text];",
  "      } else if (currentLang === 'de') {",
  "        for (var de in dict) {",
  "          if (dict[de] === opt.text) { opt.text = de; break; }",
  "        }",
  "      }",
  "    }",
  "  });",
  ...
  "  ",
  "  // Translate the page multiple times to catch dynamic content",
  "  translatePage();",
  ...
  "  });",
  "  ",
  "  // Set up observer to watch for page changes and retranslate",
  "  var observer = new MutationObserver(function(mutations) {",
  "    // Only retranslate if we're in English mode and something changed",
  "    if (currentLang === 'en') {",
