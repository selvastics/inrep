...
custom_page_flow <- list(
  ...
  list(
    ...
    content = '<div style="padding: 20px; text-align: center;">
      ...
      <p>Welcome to the HilFo Study</p>
      <label style="display: block; margin: 20px 0;">
        <input type="checkbox" id="consent_check" required> 
        Ich stimme der Teilnahme zu / I agree to participate
      </label>
    </div>',
    validate = "function(inputs) { 
      try {
        var deCheck = document.getElementById('consent_check');
        return Boolean(deCheck && deCheck.checked);
      } catch(e) {
        return false;
      }
    }",
