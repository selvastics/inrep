...
create_hilfo_report <- function(responses, item_bank, demographics = NULL, session = NULL) {
  ...
  download_section_html <- paste0(
    ...
    # PDF Download Button - Simplified and robust
    '<button class="btn-primary" style="margin: 5px;" onclick="downloadPDF()">',
    if (current_lang == "en") 'Save as PDF' else 'Als PDF speichern',
    '</button>',
    
    # CSV Download Button - Simplified and robust
    '<button class="btn-success" style="margin: 5px;" onclick="downloadCSV()">',
    ...
    function downloadPDF() {
      try {
        ...
        content += "Universität Hildesheim\\n";
        
        var blob = new Blob([content], {type: "text/plain;charset=utf-8"});
        var link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.download = "', pdf_filename, '.txt";
    ...
    function downloadCSV() {
      try {
        ...
        csv += "Programming Anxiety;", sprintf("%.2f", scores$ProgrammingAnxiety), "\\n";
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (currentLang === "en") {
    ...
    // Update welcome button text span if present
    var welcomeBtnText = document.getElementById("lang_switch_text");
    if (welcomeBtnText) {
      welcomeBtnText.textContent = "Deutsche Version";
    }
