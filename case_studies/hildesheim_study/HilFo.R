    ...
    function toggleLanguage() {
      ...
      if (deContent && enContent) {
        if (deContent.style.display === "none") {
          ...
          /* Store language preference (no Shiny communication to prevent loops) */
          window.hilfoGlobalLanguage = "de";
...
window.toggleLanguage = function() {
  ...
  /* Store language preference for assessment pages (no Shiny to prevent loops) */
  window.hilfoGlobalLanguage = currentLang;
  sessionStorage.setItem("hilfo_global_language", currentLang);
...
/* Assessment Question Mappings for Full HILFO Study */
window.hilfoQuestionMappings = {
  // Programming Anxiety questions (20 items)
...
/* Assessment Translation Function (only for non-page-1 pages) */
function translateAssessmentPage() {
  // Don't translate page 1 - it has its own perfect system
  var isPage1 = document.getElementById("content_de") && document.getElementById("content_en");
  if (isPage1) {
    return; // Skip page 1 completely
  }
  
  var currentLang = window.hilfoGlobalLanguage || sessionStorage.getItem("hilfo_global_language") || "de";
  ...
  if (isEnglish) {
    ...
    selectors.forEach(function(selector) {
      ...
      elements.forEach(function(el) {
        ...
        if (text && window.hilfoQuestionMappings[text]) {
          el.textContent = window.hilfoQuestionMappings[text];
          console.log("Assessment translated:", text, "->", window.hilfoQuestionMappings[text]);
        }
      });
    });
  }
...
/* Create language button for assessment pages only */
function createAssessmentLanguageButton() {
  // Don't add button to page 1 - it has its own
  var isPage1 = document.getElementById("content_de") && document.getElementById("content_en");
  if (isPage1) {
    return;
  }
  
  if (document.querySelector(".assessment-language-btn")) {
    return; // Already exists
  }
  
  var btn = document.createElement("button");
  btn.className = "assessment-language-btn";
  btn.type = "button";
  btn.onclick = function() {
    ...
    window.hilfoGlobalLanguage = newLang;
    sessionStorage.setItem("hilfo_global_language", newLang);
    
    // Update button text
    btn.innerHTML = "<span>" + (newLang === "en" ? "Deutsche Version" : "English Version") + "</span>";
    
    // Translate current page
    translateAssessmentPage();
    
    // Notify inrep system (only from assessment pages)
  ...
  var currentLang = window.hilfoGlobalLanguage || sessionStorage.getItem("hilfo_global_language") || "de";
  btn.innerHTML = "<span>" + (currentLang === "de" ? "English Version" : "Deutsche Version") + "</span>";
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  createAssessmentLanguageButton();
  
  /* Translate assessment pages if language is already set to English */
  setTimeout(function() {
    translateAssessmentPage();
  }, 300);
...
var observer = new MutationObserver(function(mutations) {
  createAssessmentLanguageButton();
  // Translate new content if language is English
  setTimeout(function() {
    translateAssessmentPage();
  }, 100);
});
