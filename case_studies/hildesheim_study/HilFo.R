...
// Navigation debouncing to prevent double-click page skips
function setupNavigationDebouncing() {
  // Debounce navigation buttons to prevent double-clicking
  var navigationButtons = document.querySelectorAll('button[type="submit"], .btn-primary, .btn-success, .btn-info, .btn-warning, .btn-danger, .btn-secondary, .btn-light, .btn-dark, .btn-link, input[type="submit"], input[type="button"]');
  ...
  navigationButtons.forEach(function(button) {
    ...
    button.addEventListener('click', function(e) {
      ...
      button.innerHTML = '<i class="fas fa-spinner fa-spin" style="margin-right: 8px;"></i>' + 
                        (button.textContent.includes('Next') || button.textContent.includes('Weiter') ? 'Loading...' : 
                         button.textContent.includes('Back') || button.textContent.includes('Zurück') ? 'Loading...' : 'Processing...');
      ...
      // Re-enable after 1.5 seconds with original appearance
      setTimeout(function() {
...
// Also set up debouncing when page loads
document.addEventListener('DOMContentLoaded', function() {
  setupNavigationDebouncing();
});

// Global click interceptor for all clickable elements
document.addEventListener('click', function(e) {
  var target = e.target;
  
  // Check if the clicked element is a navigation button
  if (target.tagName === 'BUTTON' || target.tagName === 'INPUT' || target.classList.contains('btn')) {
    // Check if this is a navigation-related button
    var buttonText = (target.textContent || target.value || '').toLowerCase();
    var isNavigationButton = buttonText.includes('next') || buttonText.includes('weiter') || 
                           buttonText.includes('back') || buttonText.includes('zurück') ||
                           buttonText.includes('submit') || buttonText.includes('continue') ||
                           buttonText.includes('weiter') || buttonText.includes('fortfahren') ||
                           target.type === 'submit' || target.classList.contains('btn-primary') ||
                           target.classList.contains('btn-success');
    
    if (isNavigationButton && !canNavigate()) {
      e.preventDefault();
      e.stopPropagation();
      ...
      if (target.tagName === 'BUTTON') {
        ...
        target.innerHTML = '<i class="fas fa-exclamation-triangle" style="margin-right: 8px;"></i>Please wait...';
...
var navigationState = {
  ...
  minNavigationInterval: 1000 // Minimum 1 second between navigations
};

// Check if navigation is allowed
function canNavigate() {
  var now = Date.now();
  var timeSinceLastNavigation = now - navigationState.lastNavigationTime;
  
  if (navigationState.isNavigating || timeSinceLastNavigation < navigationState.minNavigationInterval) {
    return false;
  }
...
function startNavigation() {
  ...
  if (navigationState.timeout) {
    clearTimeout(navigationState.timeout);
  }
  
  // Reset navigation flag after 2 seconds
  navigationState.timeout = setTimeout(function() {
    navigationState.isNavigating = false;
  }, 2000);
}

// Intercept form submissions to prevent double-submission
...
// Intercept Shiny input changes that might trigger navigation
if (typeof Shiny !== "undefined") {
  var originalSetInputValue = Shiny.setInputValue;
  Shiny.setInputValue = function(name, value, options) {
    // Check if this is a navigation-related input
    if (name && (name.includes('page') || name.includes('next') || name.includes('back') || name.includes('submit'))) {
      if (!canNavigate()) {
        console.log('Shiny navigation blocked - too soon or already navigating');
        return;
      }
      startNavigation();
    }
    ...
    return originalSetInputValue.call(this, name, value, options);
  };
}
