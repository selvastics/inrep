...
# CRITICAL for shinyapps.io: Explicitly load httr and jsonlite for WebDAV uploads
# These packages MUST be loaded in global environment for save_session_to_cloud to work
...
custom_item_selection <- function(rv, item_bank, config, session = NULL) {
  ...
  # CRITICAL: Read administered items from BOTH session$userData AND rv$administered
  # WHY: Non-adaptive pages (like page 6) populate rv$administered, not session$userData$administered
  # Adaptive pages populate session$userData$administered (to avoid reactive re-renders)
  # We need to merge BOTH to get complete picture
  administered_from_session <- if (!is.null(session) && !is.null(session$userData$administered)) {
    session$userData$administered
  } else {
    integer(0)
  }
  
  administered_from_rv <- rv$administered %||% integer(0)
  
  # Merge both sources - union ensures we don't double-count
  administered_items <- unique(c(administered_from_session, administered_from_rv))
  
  cat("DEBUG: administered_from_session:", paste(administered_from_session, collapse=", "), "\n")
  cat("DEBUG: administered_from_rv:", paste(administered_from_rv, collapse=", "), "\n")
  cat("DEBUG: merged administered_items:", paste(administered_items, collapse=", "), "\n")
  ...
  # Check responses for PA items - DEFENSIVE: handle empty/NULL rv$responses
  responses_for_pa <- if (!is.null(rv$responses) && length(rv$responses) >= 23) {
    which(!is.na(rv$responses[1:23]))
  } else if (!is.null(rv$responses) && length(rv$responses) > 0) {
    which(!is.na(rv$responses[1:min(23, length(rv$responses))]))
  } else {
    integer(0)
  }
  ...
  if (effective_pa_count >= 5 && effective_pa_count < 10) {
    ...
    # Get responses so far - DEFENSIVE: handle NULL or empty
    responses_so_far <- if (!is.null(rv$responses) && length(rv$responses) > 0) {
      rv$responses[1:length(rv$responses)]
    } else {
      numeric(0)
    }
    
    # Count ACTUAL non-NA responses
    num_valid_responses <- sum(!is.na(responses_so_far))
    message(sprintf("Responses collected so far: %d items", num_valid_responses))
    ...
    # CRITICAL: Only estimate if we have at least 3 VALID (non-NA) responses!
    if (num_valid_responses >= 3 && 
